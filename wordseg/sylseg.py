"""
Find landmark segments from given audio.
...
import gammatone.filters # http://detly.github.io/gammatone/filters.html
import torch
import torchaudio
...
def get_boundaries(wav_fn, return_outh=False, fs=None):
    ...
    # Read the audio data TODO make so only read in the audio at 16000Hz, not the path
    if isinstance(wav_fn, str) or isinstance(wav_fn, Path):
        wav_data, fs = torchaudio.load(wav_fn, backend='soundfile')
    else:
        wav_data = wav_fn
        fs = fs
    ...
    filtered_signal = gammatone.filters.erb_filterbank(wav_data.squeeze(), coefs)
